--[[
    ...
    Fitur sama kayak Server Hop di panel + filter "Min Players in Target"
    (cari server yang isinya LEBIH dari X). Jalan sendiri tanpa panel.

    Auto-execute LOCAL: source di-embed di bawah (readfile executor cuma bisa
    baca workspace, bukan path absolut), ditulis ke StealAnEgg/autoHop.lua
    tiap kali script di-run, dan queueonteleport-nya load dari file lokal itu.
...
local SELF_SRC = [==[
--[[ AUTO HOP core - salinan lokal buat auto-execute ]]
-- single-instance guard: kalo script ke-execute ulang (instance baru),
-- instance lama langsung berhenti - biar ga ada 2 chat loop / 2 hop loop
-- yang nulis label + kirim chat barengan (itu biang "chat off" tiba2).
...
local SELF_PATH  = CFG_FOLDER .. "/autoHop.lua"
local DISK_PATH  = [[D:\Roblox\Steal an Egg\AutoHop.lua]]
...
local function fetchServers(noFilter)
    local list = {}
...
local function doHop()
    ...
    local cands = {}
    for _, s in ipairs(servers) do cands[#cands + 1] = s end
    if #cands == 0 then
        cands = fetchServers(true)
    end
    if #cands == 0 then
        hopping = false
...
task.spawn(function()
    ...
    task.wait(2)
    local run = function(src)
        if type(src) == "string" and #src > 0 then pcall(function() loadstring(src)() end) end
    end
    -- prioritas: file di D: langsung (bukan salinan workspace executor)
    local tried = false
    if type(loadfile) == "function" then
        local ok, fn = pcall(loadfile, %q)
        if ok and type(fn) == "function" then tried = true; pcall(fn) end
    end
    if not tried and type(dofile) == "function" then
        local ok, res = pcall(dofile, %q)
        tried = ok and res ~= nil
    end
    if not tried and type(readfile) == "function" then
        local ok, src = pcall(readfile, %q)
        if ok and type(src) == "string" and #src > 0 then tried = true; run(src) end
    end
    if not tried and type(readfile) == "function" then
        local ok, src = pcall(readfile, %q)
        if ok and type(src) == "string" and #src > 0 then run(src) end
    end
end)
]]):format(DISK_PATH, DISK_PATH, DISK_PATH, SELF_PATH)
end
...
do -- bunuh SEMUA panel lama di semua root (PlayerGui + CoreGui + gethui),
   -- biar ga ada 2 panel numpuk yang satu nulis "chat off" versi lama
    local seen = {}
...
print("[AutoHop] core loaded - standalone server hopper")
]==]
...
local S = {
    ...
    hopMaxPlayers   = 7,              -- target harus <= ini
    hopMinPlayers   = 0,              -- target harus > ini (0 = off)
    hopPreferEmpty  = true,
    autoChat        = false,
    chatText        = "",
    chatText2       = "",
...
local function doHop()
    if hopping then return false end
    hopping = true
    armAutoExec() -- selalu re-inject diri sendiri di server baru (dari file lokal)
    local servers = fetchServers()
    local cands = {}
    for _, s in ipairs(servers) do cands[#cands + 1] = s end
    if #cands == 0 then
        -- ga ada yang lolos filter min/max: fallback ke semua server publik
        cands = fetchServers(true)
    end
    if #cands == 0 then
        hopping = false
...
-- ==================== auto-execute LOCAL ====================
-- loader nge-load salinan lokal dari workspace executor (bukan URL remote)
local function loaderSrc()
    return ([[
if getgenv().__AUTOHOP_RUN then return end
getgenv().__AUTOHOP_RUN = true
...
-- tulis salinan lokal (selalu refresh biar file workspace = file ini)
pcall(function()
    if type(makefolder) == "function" then pcall(makefolder, CFG_FOLDER) end
    if type(writefile) == "function" and type(SELF_SRC) == "string" then
        writefile(SELF_PATH, SELF_SRC)
    end
end)
...
task.spawn(function()
    while getgenv().__AUTOHOP_ACTIVE == MY_RUN do
        if S.autoHop and hopReady() then
            ...
            -- kalo ga nemu server yang cocok: jangan reset baselines,
            -- loop bakal cek lagi 5 detik kemudian
        end
        task.wait(5)
    end
end)
...
do -- hapus panel lama (kalau ada)
    for _, g in ipairs(CoreParent:GetChildren()) do
        if g.Name == "AutoHop_Panel" then pcall(function() g:Destroy() end) end
    end
end
...
print("[AutoHop] loaded - standalone server hopper (local auto-execute)")
